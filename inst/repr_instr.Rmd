...
Note that to re-run this analysis you'll need an anonymized copy of the data available in a subdirectory of the working directory. You can obtain the anonymized data from the project repository at <https://osf.io/nvugt/>.
...
where `path-to-anonymized-data` is replaced with the name of the subdirectory with the anonymous data (or one of the simulated versions of the data). Note that the inferential statistics can take very long (~24 hours) to run. This will save a file `analysis.html` with the analysis report and a binary image `analysis.RData` containing the analysis data.
