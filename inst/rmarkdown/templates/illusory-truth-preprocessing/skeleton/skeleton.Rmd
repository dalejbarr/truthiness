---
title: "The Trajectory of Truth: Data Preprocessing and Anonymization"
...
if (!dir.exists(params$anondir)) {
  stop("subdirectory '", params$anondir, "' does not exist")
}
...
man_exclude_part_fname <- paste0(basename(params$anondir),
                                 "_manually_exclude_participants.csv")

man_exclude_phs_fname <- paste0(basename(params$anondir),
                                 "_manually_exclude_phases.csv")
...
params$anondir <- normalize_path(params$anondir) # no trailing slash
if (dir.exists(params$anondir)) {
  unlink(params$anondir, TRUE, TRUE)
}
dir.create(params$anondir)
if (check_fake(params$subdir)) {flag_fake(params$anondir)}
...
The raw data files contain `r .pi(nrow(ratings))` truth ratings of `r
.pi(n_item)` statements from `r .pi(nrow(sess))` participants.
...
- **PHASES DROPPED DUE TO SUBJECT EXCLUSIONS: 
...
```{r anonymize, eval = (complete_data), results = 'asis'}
...
readr::write_csv(sess_share, file.path(params$anondir, "ANON_sessions.csv"))
readr::write_csv(phase_share, file.path(params$anondir, "ANON_phases.csv"))
readr::write_csv(ratings_share, file.path(params$anondir, "ANON_ratings.csv"))
readr::write_csv(cjudgments_share, file.path(params$anondir, "ANON_cjudgments.csv"))

cat("\nWrote anonymized data to files ",
    "`ANON_sessions.csv`, `ANON_phases.csv`, `ANON_ratings.csv`, and ",
    "`ANON_cjudgments.csv` in subdirectory `", params$anondir, "`.\n", sep = "")

mp_outfile1 <-
  file.path(basename(params$anondir), "manually_exclude_participants.csv")
mp_outfile2 <- 
  file.path(basename(params$anondir), "manually_exclude_phases.csv")

write_csv(man_exclude_part, mp_outfile1)
write_csv(man_exclude_phs, mp_outfile2)
          
cat("\nCopied manual exclusions to `",
    mp_outfile1, "` and `", mp_outfile2, "`.\n", sep = "")
```
