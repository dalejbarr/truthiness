...
readr::write_csv(cjudgments_share, file.path(anondir, "ANON_cjudgments.csv"))

cat("\nWrote anonymized data to files ",
    "`ANON_sessions.csv`, `ANON_phases.csv`, `ANON_ratings.csv`, and ",
    "`ANON_cjudgments.csv` in subdirectory `", anondir, "`.\n", sep = "")

mp_outfile1 <-
  file.path(basename(anondir), "manually_exclude_participants.csv")
mp_outfile2 <- 
  file.path(basename(anondir), "manually_exclude_phases.csv")
