...
## man_exclude_part_fname <- paste0(basename(params$anondir),
man_exclude_part_fname <- "manually_exclude_participants.csv"

## man_exclude_phs_fname <- paste0(basename(params$anondir),
man_exclude_phs_fname <- "manually_exclude_phases.csv"
...
private_sess_fname <- paste0(basename(params$anondir),
                             "_NOT_ANONYMIZED_sessions.rds")
private_phase_fname <- paste0(basename(params$anondir),
                              "_NOT_ANONYMIZED_phases.rds")
report_fname <- paste0(basename(params$anondir), "_preprocessing_report.html")
...
## Participants who logged multiple sessions
...
sess_mult %>%
  inner_join(phase, "PID") %>%
  select(PID, phase_id, StartDate, EndDate, Finished) %>%
  arrange(PID, phase_id, StartDate) %>%
  DT::datatable()
```

There were `r nrow(sess_mult)` total participants who logged multiple sessions in the system.
...
ratings <- semi_join(ratings_pre,
                     sess_consent %>%
                     filter(chk_consent_all),
                     "PID")

cjudgments <- semi_join(cjudgments_pre,
                        sess_consent %>%
                        filter(chk_consent_all),
                        "PID")
...
tspt <- split(ratings[["trating"]],
              list(ratings[["PID"]], as.character(ratings[["phase_id"]])),
              sep = ",")
...
phs_flat <- left_join(phs_flat_t, phs_flat_c,
                      c("PID", "phase_id")) %>%
...
phase_keep <- semi_join(phase_man,
                        sess_keep %>% filter(chk_consent_all), "PID") %>%
...
```{r dropouts-sum, message=FALSE, results = 'asis'}
phases <- as.integer(phase %>% distinct(phase_id) %>% pull() %>% sort())
.this_phase <- max(phases)

.s2 <- sess_keep %>%
  group_by(PID) %>%
  summarize(chk_consent_all = all(chk_consent_all),
            chk_notmanex_sess = all(chk_notmanex),
            chk_native = all(chk_native), .groups = "drop")

phase_man %>%
  semi_join(.s2 %>%
            filter(chk_consent_all & chk_notmanex_sess & chk_native), "PID") %>%
  filter(!(chk_dur_phase & chk_finished)) %>%
  count(phase_id)

.p2 <- phase_man %>%
  mutate(chk_nocheat = !grepl("^Yes", Cheat, ignore.case = TRUE)) %>%
  left_join(phs_flat, c("PID", "phase_id")) %>%
  replace_na(list(chk_noflatline = FALSE)) %>%
  group_by(PID, phase_id) %>%
  summarize(chk_consent = all(chk_consent),
            chk_finished = all(chk_finished),
            chk_notmanex = all(chk_notmanex),
            chk_dur_phase = all(chk_dur_phase),
            chk_nocheat = all(chk_nocheat),
            chk_noflatline = all(chk_noflatline),
            .groups = "drop")

.sp <- left_join(.s2, .p2, "PID") %>%
  replace_na(list(phase_id = "1",
                  chk_consent = FALSE,
                  chk_finished = FALSE,
                  chk_notmanex = FALSE)) %>%
  mutate(phase_keep = chk_consent & chk_finished & chk_notmanex &
           chk_nocheat & chk_noflatline & chk_dur_phase,
         sess_keep = chk_consent_all & chk_notmanex_sess & chk_native,
         keep = phase_keep & sess_keep)

.drop1 <- .sp %>%
  group_by(phase_id) %>%
  summarise(n_attempted = n(),
            n_retained = sum(keep),
            n_excluded = n_attempted - n_retained,
            .groups = "drop") %>%
  select(phase_id, n_attempted, n_excluded, n_retained)

.drop1 %>%
  inner_join(bind_rows(tibble(phase_id = factor("1", levels = 1:4),
                              n_invited = NA_integer_),
                       .drop1 %>%
                       mutate(phase_id = factor(as.integer(phase_id) + 1L,
                                                levels = 1:4)) %>%
                       select(phase_id, n_invited = n_retained)),
             "phase_id") %>%
  select(phase_id, n_invited, n_attempted, n_excluded, n_retained) %>%
  mutate(dropout = sprintf("%0.1f%%",
                           100 * (n_invited - n_attempted) / n_invited),
         exclusions = sprintf("%0.1f%%",
                              100 * (n_excluded / n_attempted))) %>%
  DT::datatable()
```

```{r recruit-txt, results = 'asis'}
## only run this if we have more phases to run
if (.this_phase < 4L) {
  next_phase <- .this_phase + 1L

  cat("\n## Recruitment for phase ", next_phase, "\n\n", sep = "")

  .np1 <- phase_keep %>%
    filter(phase_id == max(phases), keep) %>%
    semi_join(sess_keep %>% filter(keep), "PID") %>%
    select(PID, list_id) %>%
    arrange(list_id, PID)

  .np2 <- .sp %>%
    filter(phase_id == .this_phase, keep) %>%
    inner_join(sess %>% select(PID, list_id), "PID") %>%
    select(PID, list_id)

  .np <- .np2
  ## old algorithm vs new
  ## if there are people that wouldn't be excluded, check multiples
  if (nrow(setdiff(.np1, .np2)) > 0L) {
    .in_common <- semi_join(setdiff(.np1, .np2), sess_mult, "PID")
    ## see if we can explain the difference
    if (nrow(.in_common) != nrow(setdiff(.np1, .np2, "PID"))) {
      warning("old and new exclusion algorithms don't match; using old algorithm")
      .np <- .np1
    }
  }
  
  .of <- paste0("recruitment_phase_", next_phase, ".csv")
  
  .np %>%
    write_csv(.of)

  cat("**List of prolific IDs written to file `", .of, "`**\n", sep = "")
}
```
...
sess_no_dups <- sess_keep %>%
  count(PID) %>%
  filter(n == 1L) %>%
  select(-n) %>%
  inner_join(sess_keep %>% select(PID, ID, chk_consent_all), "PID") %>%
  filter(chk_consent_all) %>%
