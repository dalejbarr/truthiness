...
if (!requireNamespace("ez")) {
  stop("You must install the 'ez' package to run this analysis.")
}
...
warn <- function() {
  if (truthiness::check_fake(params$subdir)) {
    extra_text <-
      case_when(basename(params$subdir) == "all_null" ~ "**This document demonstrates a null main effect and null interaction.**",
                basename(params$subdir) == "main_effect" ~ "**This document demonstrates a significant main effect and null interaction.**",
                basename(params$subdir) == "interaction" ~ "**This document demonstrates a significant main effect and significant interaction.**",
                TRUE ~ "")
    paste("\n<div class=\"warn\">",
...
## 'exclusions_participant.csv', created in the pre-processing stage
pfile <- file.path(basename(params$subdir), "exclude_participants.csv")
if (!file.exists(pfile)) {
  warning("File '", pfile, "' not found!")
  par_exclude <- tibble(ID = character(0), Reason = character(0))
} else {
  par_exclude <- read_csv(pfile, col_types = "cc")
}

sess_keep0 <- sess_dur_all %>%
  anti_join(par_exclude, "ID")
...
## finally, apply phase-level manual exclusions from
## 'exclusions_phase.csv', created in the pre-processing stage
pfile <- file.path(basename(params$subdir), "exclude_phases.csv")
if (!file.exists(pfile)) {
  warning("File '", pfile, "' not found!")
  phs_exclude <- tibble(ID = character(0),
                        phase_id = factor(integer(0), levels = 1:4),
                        Reason = character(0))
} else {
  phs_exclude <- read_csv(pfile, col_types = "cic") %>%
    mutate(phase_id = factor(phase_id, levels = 1:4))
}

phase_keep <- phase_fin %>%
  anti_join(phs_exclude, c("ID", "phase_id"))
...
```{r fit-clmm-no-parallel, eval = (!parallel_installed), echo = (!parallel_installed), cache=TRUE, cache.path=(cache_dir), cache.vars="mods", dependson=c(3:4, 6:7, 9)}
...
```{r fit-clmm-parallel, eval = parallel_installed, echo = parallel_installed, cache=TRUE, cache.path=(cache_dir), cache.vars="mods", dependson=c(3:4, 6:7, 9), results = 'asis'}
...
```{r validate-model, fig.cap="*Model validation: Plot of observed participant/stimulus means (points) against simulated data (violins) and cell means (black points, line).*", fig.width=10, fig.height = 5, fig.align="center"}
