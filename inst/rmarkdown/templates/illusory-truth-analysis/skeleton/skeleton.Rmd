...
simulated <-
  if (is.null(params$simulated)) {
    warning("document parameter `simulated` undefined; assuming simulated data")
    TRUE
  } else {
    params$simulated
  }
...
```{r subdir, echo = FALSE}
## if no subdirectory provided, use the most recent one with name in
## the format anon-YYYY-MM-DD
subdir <-
...
There was one exclusion criterion applying at the phase-level: namely, whether the participant completed all of the ratings in the phase.
...
cat("**NOTE**: Because the estimation of the cumulative logit mixed model ",
