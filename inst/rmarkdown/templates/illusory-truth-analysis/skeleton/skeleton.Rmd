...
```{r excl-phase-and-gender, message=FALSE}
subj_excl <- sessions %>%
  filter(!keep) %>%
...
```{r mods-included, include=FALSE}
## load in the data if provided
mods <- if (!is.null(params$modfits)) {
          readRDS(params$modfits)
        } else {
          NULL
        }
```

```{r fit-clmm-no-parallel, eval = (!parallel_installed) & is.null(mods), echo = (!parallel_installed)}
...
```{r fit-clmm-parallel, eval = parallel_installed & is.null(mods), echo = parallel_installed, results = 'asis'}
...
```{r save-image, eval=is.null(params$modfits), include=FALSE}
