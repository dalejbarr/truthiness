...
ratings2 <- ratings %>%
  semi_join(sess_keep0 %>% filter(keep), "ID")
...
.p1 <- inner_join(excluded, included, c("phase_id", "gender")) %>%
...
.psum <- .p1 %>%
  ...
  summarize(n_recruited = sum(n_recruited),
            n_excluded = sum(n_excluded),
            n_analysed = sum(n_analysed)) %>%
  ungroup() %>%
  mutate(gender = factor("**TOTAL**",
                         levels = c(levels(.p1$gender), "**TOTAL**")))

levels(.p1$gender) <- levels(.psum$gender)
...
```{r mod-setup, echo=FALSE, cache=TRUE, cache.path=(cache_dir), dependson=c(3:4, 6, 8, 10)}
...
```{r fit-clmm-no-parallel, eval = (!parallel_installed), echo = (!parallel_installed), cache=TRUE, cache.path=(cache_dir), cache.vars="mods", dependson=c(3:4, 6, 8, 10, 16)}
...
```{r fit-clmm-parallel, eval = parallel_installed, echo = parallel_installed, cache=TRUE, cache.path=(cache_dir), cache.vars="mods", dependson=c(3:4, 6, 8, 10, 16), results = 'asis'}
...
modsfit <- list(mt1 = mt1, mt2 = mt2, mt3 = mt3, mt4 = mt4, mt5 = mt5, mt6 = mt6)
saveRDS(modsfit, "fitted_models.rds")

## to load:
## mf <- readRDS("fitted_models.rds")
## mt1 <- mf$mt1; mt2 <- mf$mt2; mt3 <- mf$mt3
## mt4 <- mf$mt4; mt5 <- mf$mt5; mt6 <- mf$mt6
```
