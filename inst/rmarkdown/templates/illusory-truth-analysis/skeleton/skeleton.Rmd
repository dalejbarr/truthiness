...
```{r reproduction-recipe, echo = FALSE, eval = FALSE, results = 'asis'}
...
library("emmeans")
library("ordinal")
## TODO change this line to library("truthiness")
...
```{r fit-clmm-no-parallel, eval = (!parallel_installed), echo = (!parallel_installed), cache=TRUE, cache.path=(cache_dir), cache.vars="mods", dependson=c(4:8)}
...
```{r fit-clmm-parallel, eval = parallel_installed, echo = parallel_installed, cache=TRUE, cache.path=(cache_dir), cache.vars="mods", dependson=c(4:8), results = 'asis'}
